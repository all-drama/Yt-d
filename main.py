    ...
    if st.button("Extract Video URL", key="extract"):
        if url:
            ...
            if video_url:
                ...
                st.markdown(
                    f'<div class="rainbow-button">{st.button("Download Video", key="download")}</div>',
                    unsafe_allow_html=True
                )
                if st.button("Download Video", key="download"):
                    webbrowser.open(video_url)
                    # You can also use the following line to open the URL in a new tab:
                    # webbrowser.open_new_tab(video_url)
                    # Please note that opening a new tab may depend on the user's browser settings.
            else:
