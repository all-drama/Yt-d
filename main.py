        ...
        if url:
            ...
            if video_url:
                ...
                if download_video(video_url, save_dir):
                    ...
                    st.text(f"Video downloaded and saved at: {save_path}")
                    if os.path.exists(save_path):
                        download_button_str = get_binary_file_downloader_html(save_path, 'Video')
                        st.markdown(download_button_str, unsafe_allow_html=True)
                    else:
                        st.error("Failed to locate the downloaded video file.")
                else:
